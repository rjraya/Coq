...
(*
(* People may say Isabelle is strange, but they do this much better *)

Fixpoint lexicographic (n m: nat) :=
  match n, m with
  | O, O => O
  | O, x => lexicographic O (x-1)
  | x, O => lexicographic (x-1) O
  | x, y => lexicographic x (y-1)
  end.
*)
...
Proof. reflexivity.  Qed.
Example test_bin_to_nat5:           bin_to_nat (A (A (B Z))) = 4.
Proof. reflexivity.  Qed.
...
Theorem incr_to_nat : forall (m:bin), bin_to_nat (incr m) = bin_to_nat m+1.
Proof. 
  intros m.
  induction m.
  - reflexivity.
