...
Proof.
  split.
  - intros H. induction H as [| | n m H3 H4 H5 H6]. 
    + apply ev_0.
    + apply ev_SS. apply ev_0.
    + apply ev_sum. apply H4. apply H6.
  - intros. induction H as [| n' E' IHn].
    + apply even'_0. 
    + assert(lem1: forall (m:nat), S (S m) = m+2).
      { induction m. reflexivity. simpl. rewrite IHm. reflexivity. }
      rewrite lem1.
      apply (even'_sum n' 2 IHn even'_2). 
Qed.    
(** [] *)
...
Proof.
  intros n m H1 H2.
  induction H2 as [| n' E' IHn].
  - simpl in H1. apply H1.
  - simpl in H1. inversion H1 as [| l Ehl Eql]. apply (IHn Ehl).
Qed.
(** [] *)
...
Proof.
  ...
  apply ev_even in H2.
  destruct H1 as [k kH].
  destruct H2 as [k' k'H].
  rewrite ev_even_iff. 
  exists (k + k' - n).
  rewrite double_plus.
  rewrite double_plus in kH.
  rewrite double_plus in k'H.
  omega.
Qed.
(** [] *) 
...
Proof.
 ...
 - destruct IHle as [IHle1 IHle2]. split. 
  + apply le_S. apply IHle1.
  + apply le_S. apply IHle2.
Qed.
...
Proof.
  ...
  - destruct n. 
    ...
    + intros H. simpl in H. apply n_le_m__Sn_le_Sm. 
      apply IHm. apply H.
Qed.
...
Proof.
  ...
  generalize dependent n.
  induction m.
  - intros n H. destruct n. reflexivity. inversion H.
  - intros n H. destruct n. reflexivity. simpl. apply Sn_le_Sm__n_le_m in H.
    apply IHm. apply H.
Qed.
...
(** - Which of the following propositions are provable?
    ...
    - R 0 0 0 -> R 1 0 1 -> R 1 1 2
      R 0 0 0 -> R 1 0 1 -> R 2 0 2 -> R 0 2 2 -> ...
      is impossible to deduce the second one. The sum of the first two 
      has to be equal to the second.

    - No, R 0 0 0 -> ... -> R m n o -> R n m o can be simulated by changing
      the deduction interchanging c2 and c3 (c4 is symmetric).

    - R 0 0 0 -> ... -> R (S m) (S n) (S (S o)) -> R m n o
      In the system we can deduce everything so that the sum of the first 
      two equals the third. It suffices to directly derive R m n o.
...
Proof.
  ...
  - intros H.
    ...
    + simpl in IHR. rewrite <- plus_n_Sm in IHR. injection IHR as H'.
      apply H'.
  ...
  - intros H.
    ...
    induction m. 
    + induction n. 
      ++ intros n H. simpl in H. rewrite <- H. apply c1.
      ++ intros o H. simpl in H. rewrite <- H. apply c3. 
          apply IHn. reflexivity.
    + induction n. 
      ++ intros o H. rewrite <- plus_n_O in H. rewrite <- H. 
...
Proof.
  ...
  induction H.
  - apply nil_s.
  - simpl. apply cons_in. apply IHsubseq. 
  - simpl. apply cons_nin. apply IHsubseq. 
Qed. 

(*

  (* 
   Interesting lemma illustrating the problem of induction
   over inductive datatypes in Coq.
  *)
...
Proof.
  ...
  induction s23 as [ l2' | l2' l3' x H IH | l2' l3' x H IH].
  - intros l1 s12. inversion s12. apply nil_s. 
  - intros l1 s12. 
    inversion s12.
    + apply nil_s.
    + apply cons_in.
...
(* - R 0 [] -> R 1 [0] -> R 2 [1;0]
   - R 0 [] ->* R 2 [1;0] -> R 3 [2;1;0] -> R 2 [2;1;0] -> R 1 [2;1;0] -> 
          -> R 2 [1;2;1;0] -> R 1 [1;2;1;0]
   - It is impossible, the first element of the list h satisfies 
     that h+1 >= i where i is the first index of R.
...
Lemma regex_list_id : forall T (s: list T), s =~ reg_exp_of_list s.
Proof. 
  induction s.
  - simpl. apply MEmpty.
  - simpl. 
    replace (x::s) with ([x]++s).
    apply MApp.
    + apply MChar.
    + apply IHs.
    + reflexivity.
...
Proof.
  ...
  - intros H1.
    ...
    induction s2.
    ++ intros s1 H1. simpl in H1. inversion H1. reflexivity.
    ++ intros s1 H1. simpl in H1. inversion H1. inversion H3.
       simpl. apply IHs2 in H4. rewrite H4. reflexivity.
  - intros H1.
    generalize dependent s1. 
    induction s2. 
    + intros s1 H1. simpl. rewrite H1. apply MEmpty.
    + intros s1 H1. simpl. rewrite H1. 
      replace (x::s2) with ([x] ++ s2).
      ++ apply MApp.
        +++ apply MChar.
        +++ apply regex_list_id.
      ++ reflexivity.
...
Proof.
  ...
  - intros H.
    ...
    + simpl in H. rewrite andb_true_iff in H. destruct H as [H1 H2].
      ...
      exists (x ++ y). apply MApp. apply H1. apply H2.
    + simpl in H. rewrite orb_true_iff in H. 
      destruct H.
      ++ apply IHre1 in H. destruct H as [x H]. exists x. apply MUnionL. apply H.
      ++ apply IHre2 in H. destruct H as [y H]. exists y. apply MUnionR. apply H.
    + exists []. apply MStar0.
Qed.
(** [] *)
...
Proof.
  intros. 
  remember (Star re) as re' eqn:Seq.
  induction H.
  - intros. inversion Seq.
  - intros. inversion Seq.
  - intros. inversion Seq.
  - intros. inversion Seq.
  - intros. inversion Seq.
  - intros. injection Seq as Seq.
    exists []. simpl. split.
    + reflexivity.
    + intros. exfalso. apply H.
  - intros. injection Seq as Seq. 
    destruct IHexp_match2 as [ss2 [Hss2a Hss2b]].  
    + rewrite Seq. reflexivity.
    + exists ([s1]++ss2). split.
      +++ simpl.  rewrite Hss2a. reflexivity.
      +++ intros. apply In_app_iff in H1. destruct H1 as [H1a | H1b].
          * simpl in H1a. destruct H1a as [H1a|].
            rewrite <- H1a. rewrite <- Seq. apply H.  
            exfalso. apply H1.
          * apply Hss2b. apply H1b.
Qed.
(** [] *)
...
Lemma napp_star: forall T (m: nat) (re : @reg_exp T) s,
  s =~ re -> napp m s =~ Star re.
Proof. 
  intros. 
  induction m.
  - simpl. apply MStar0.
  - simpl. apply MStarApp. apply H. apply IHm.
Qed.
...
Proof.
  ...
  - simpl. intros H. rewrite app_length in H. 
    destruct (Nat.leb (pumping_constant re1) (length s1)) eqn:L. 
    + apply Compare_dec.leb_complete in L.
      apply IH1 in L.
      destruct L as [ s2' [s3' [s4' [H1 [H2 H3]]]]].
      exists s2'. exists s3'. exists (s4'++s2).
      split. 
      ++ replace (s2' ++ s3' ++ s4' ++ s2) with ((s2' ++ s3' ++ s4') ++ s2).
         +++ rewrite <- H1. reflexivity. 
         +++ rewrite app_assoc. rewrite <- app_assoc. rewrite (app_assoc T s2' s3'). reflexivity.
      ++ split.
        +++ apply H2.
        +++ intros.
            rewrite (app_assoc T (napp m s3')). rewrite app_assoc.
            apply (MApp (s2' ++ napp m s3' ++ s4')).
            apply H3. apply Hmatch2.
    + assert(lem: leb (pumping_constant re2) (length s2) = Datatypes.true).
      { apply Compare_dec.leb_correct.
         apply leb_complete_conv in L as L. 
         omega.
      }
      Search leb.
      apply leb_iff_conv in L.
      apply Nat.leb_le in lem.
      apply IH2 in lem.   
      destruct lem as [ s1' [s3' [s4' [H1 [H2 H3]]]]].
      ...
      ++ split. 
        ...
        +++ intros. rewrite <- app_assoc.
            ...
            apply H3.
  - simpl. intros. 
    assert(lem: pumping_constant re1 <= length s1 ).
    { omega. }
    apply IH in lem.
    destruct lem as [ s1' [s3' [s4' [H1 [H2 H3]]]]].
  ...
  - simpl. intros. 
    assert(lem: pumping_constant re2 <= length s2 ).
    { omega. }
    apply IH in lem.
    destruct lem as [ s1' [s3' [s4' [H1 [H2 H3]]]]].
    exists s1'. exists s3'. exists s4'.
  ...
  - simpl. omega.
  - simpl. rewrite app_length. intros. simpl in IH2.
    destruct (length s2) eqn:Empty.
    + assert(lem: s2 = []).
      { induction s2. reflexivity. discriminate. }  
      subst. 
      rewrite <- plus_n_O in H. 
      exists []. exists s1. exists [].
      split. reflexivity. split. 
      assert(lem: s1 <> []).
      { induction s1. simpl in H. inversion H. 
        unfold not. intros. discriminate. }
      apply lem.
      intros. simpl. rewrite app_nil_r. apply napp_star. apply Hmatch1.
    + destruct IH2 as [s1' [s3' [s4' [H1 [H2 H3]]]]]. omega. 
      intros. exists (s1++s1'). exists s3'. exists s4'.
      ++ rewrite <- app_assoc. rewrite <- H1. split. 
        +++ reflexivity.
        +++ split. 
          * apply H2.
...
Proof.
  ...
  - (* l = m :: l' *)
    simpl. destruct (Basics.eqb n m) eqn:H. 
    + (* n =? m = true *)
...
Proof.
  intros P b H. split.
  - inversion H as [pH bt | npH bf].
    + reflexivity.
    + intros pH. apply npH in pH. inversion pH.
  - inversion H as [pH bt | npH bf].
    + intros. apply pH.
    + intros. discriminate.
Qed.
...
Lemma eqbP : forall n m, reflect (n = m) (Basics.eqb n m).
Proof.
...
Fixpoint count n l :=
  match l with
  | [] => 0
  | m :: l' => (if Basics.eqb n m then 1 else 0) + count n l'
  end.
...
Proof.
  ...
  - simpl. destruct (eqbP n h) as [H'' | H''].
    + simpl in H. rewrite H'' in H. rewrite <- (eqb_refl h) in H. discriminate.
    + simpl in H'. destruct H' as [H1 | H2].
      ++ unfold not in H''. apply H''. rewrite <- H1. reflexivity.
      ++ apply IHl. 
        * simpl in H. rewrite <- eqb_neq in H''. rewrite H'' in H.
          simpl in H. apply H.
        * apply H2.
Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | nost_single (x:X) : nostutter [x]
  | nost_neq (x:X) (y:X) l' (H1: x <> y) (H2: nostutter (y::l')) : nostutter (x :: y :: l')
.
...
Inductive in_merge {X:Type} : list X -> list X -> list X -> Prop :=
  | in_n_n : in_merge [] [] []
  | in_n_c l : in_merge l [] l
  | in_c_n l : in_merge [] l l 
  | in_c_c_l x l1 l2 l3 (H: in_merge l1 l2 l3) : in_merge (x::l1) l2 (x::l3)
  | in_c_c_r y l1 l2 l3 (H: in_merge l1 l2 l3) : in_merge l1 (y::l2) (y::l3)
.
...
Proof. 
  ...
  - simpl. induction l. 
    ...
    + simpl.
      ...
      rewrite H. 
      assert(filter test l = l).
      { apply IHl. intros. apply (H0 x0). simpl. right. apply H2. }
  ...
  - induction l.
    ...
    + simpl. 
      ...
      rewrite H.
      apply IHl. intros. apply H1. simpl. right. apply H2.
  - simpl. 
    assert(test x = true).
    { apply H0. simpl. left. reflexivity. }
    rewrite H2. rewrite IHin_merge. 
    + reflexivity.
    + intros. apply H0. simpl. right. apply H3.
    + apply H1.
  - simpl.
    assert(test y = false).
    { apply H1. simpl. left. reflexivity. }
    ...
    + reflexivity.
    + apply H0.
    + intros. apply H1. simpl. right. apply H3.
Qed.
...
Proof. 
  ...
  induction l.
  - simpl. apply p_nil.
  - simpl. rewrite app_assoc. apply p_cons. apply IHl.
Qed.
...
Lemma zero_length: forall (X:Type) (l: list X), length l = 0 -> l = []. 
Proof. 
  intros. 
  induction l. 
  - reflexivity.
