...
(* Rewrite performs general rewriting while apply performs unification. 
   Rewrite can be used for pure equational reasoning while apply can be used
   for unification reasoning. *)
...
Proof.
  ...
  - intros m H. destruct m.
    + discriminate.
    + simpl in H. f_equal. apply IHn. apply H.
Qed.
(** [] *)
...
(* We first do induction on n leaving m arbitrary. 
   The case where n = 0 goes by case analysis on m. 
   If m = 0 then it is immediate. If m = S m' then disjointness 
   of constructors solves the goal. 
   The case where n = S n' requires a case analysis on m.   
   The case where m = 0 goes by disjointness f constructors. 
   The case where m = S m' reduces to show m' = n'. By the induction 
   hypothesis it suffices to show n' =? m' = true since we can 
   choose the m we want (not the original m). This is obtained by 
   simplifying on the assumption. *)
...
Proof.
  ...
  - simpl. intros n H. destruct n.
    + discriminate.
    + simpl. inversion H. apply IHl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros A B p.  destruct p as [n m].  simpl.  reflexivity.  Qed.

Lemma lem1: forall X Y (l : list (X*Y)), split l = (fst (split l), snd (split l)).
Proof. intros X Y l. rewrite <- surjective_pairing. reflexivity. Qed.
...
Proof.
  ...
  induction l. 
  - reflexivity.
  - simpl. destruct x. destruct (split l) as [l1' l2'] eqn: split_eq. simpl.
    simpl in IHl. rewrite IHl. reflexivity.    
Qed.
(** [] *)
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)  
   ...
   Proof: *)
   (* There are four cases depending on n, m. All cases are trivial 
      but the last one since S n =? S m = S m =? S n reduces to the
      equation n =? m = m =? n. For that to go through we need an 
      arbitrary m. That's why we don't fix m at the beginning, otherwise
      the induction hypothesis would be n =? Sm = Sm =? n.

    [] *)

(** **** Exercise: 3 stars, standard, optional (eqb_trans)  *)
Lemma eqb_true_rev: forall n m : nat, n = m -> (n =? m) = true.
Proof. 
  induction n.
  - destruct m. reflexivity. discriminate.
  - destruct m. discriminate. intros H.   
    injection H as H1. simpl. apply IHn. apply H1.
Qed.
...
Proof.
  ...
  - simpl. intros l2. destruct l2. 
    ...
    + simpl. intros H. rewrite IHl1.
      +++ reflexivity.
      +++ injection H. intros H1. apply H1.
Qed.
...
Proof.
  ...
  - intros x0 lf H. 
    simpl in H.
    destruct (test x) eqn:test_eq. 
    + injection H as H1 H2. rewrite <- H1. apply test_eq.
