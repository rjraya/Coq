...
Theorem right_inverse: forall(X Y: Type) (l: list (X*Y)), combine (fst (split l)) (snd (split l)) = l.
Proof. 
  induction l.  
  - reflexivity.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun e => andb (evenb e) (7 <=? e)) l.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  (filter test l, filter (fun p => negb (test p)) l). 
...
Lemma map_app: forall (X Y: Type) (f : X -> Y) (l1 l2: list X), map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof. 
  induction l1. 
  - reflexivity.
...
Proof.
  induction l. 
  - reflexivity.
  - simpl. rewrite <- IHl. simpl. rewrite map_app. reflexivity.
Qed.
(** [] *)
...
(* For instance, to count the elements of a list with a fold *)
...
Example test_fold_map_1: fold_map S [1;2;3] = [2;3;4].
Proof. unfold fold_map. reflexivity. Qed.
...
Proof. 
  induction l. 
  - reflexivity.
  - simpl. unfold fold_map. simpl. fold (fold_map f l). rewrite IHl. reflexivity.
Qed.
...
Proof.
  unfold prod_curry. unfold prod_uncurry. reflexivity.
Qed.
...
Proof.
  unfold prod_curry. unfold prod_uncurry. destruct p. reflexivity.
Qed.
(** [] *)
...
If l is the empty list then its length is 0 and nth_error returns None.
If l is non-empty, then its length n is greater than 0 and thus 
  nth_error computes recursively, nth_error of its tail which has length n-1. 
  By induction hypothesis nth_error X tl (n-1) = None. 
...
Proof. unfold one. unfold mult. reflexivity. Qed.

Example mult_2 : mult zero (plus three three) = zero.
Proof. unfold three. unfold zero. unfold mult. reflexivity. Qed.

Example mult_3 : mult two three = plus three three.
Proof. unfold three. unfold two. unfold mult. reflexivity. Qed.
...
Proof. unfold exp. unfold three. unfold zero. unfold one. unfold exp. reflexivity. Qed. 

Example exp_3 : exp three two = plus (mult two (mult two two)) one.
Proof. unfold exp. unfold two. unfold plus. unfold one. reflexivity. Qed. 
