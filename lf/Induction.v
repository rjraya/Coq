...
Induction gives you an IH. Case analysis just disjoint cases.
*)
...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    Proof: as in any mathematics course.
*)
...
Proof.
  ...
  - simpl. intros H. rewrite IHp. reflexivity. rewrite H. reflexivity.
Qed.
...
Proof.
  induction n. 
  - reflexivity.
  - simpl. intros m p. rewrite IHn. rewrite plus_assoc. reflexivity.
Qed.
...
Proof.
  induction n.
  - reflexivity.
  - intros m p. simpl. rewrite IHn. rewrite mult_plus_distr_r. reflexivity.
Qed.
(** [] *)
...
Theorem bin_to_nat_pres_incr : forall (m:bin), bin_to_nat (incr m) = bin_to_nat m + 1.
Proof. 
  intros m.
  induction m.
  - reflexivity.
  ...
  - simpl. rewrite <- plus_n_O. rewrite <- plus_n_O.  
    assert(lem1: bin_to_nat m + bin_to_nat m + 1 = bin_to_nat m + (bin_to_nat m + 1)).
    { rewrite plus_assoc. reflexivity. }    
    rewrite lem1.
    rewrite <- IHm. 
    assert(lem2: bin_to_nat m + bin_to_nat (incr m) + 1 = 
                 bin_to_nat m +  1 + bin_to_nat (incr m)).
    { rewrite <- plus_assoc. 
      replace (bin_to_nat (incr m) + 1) with (1 + bin_to_nat (incr m)).
      - rewrite plus_assoc. reflexivity.
      - rewrite plus_comm. reflexivity.
    }
    rewrite lem2.
...
Lemma sn_to_s: forall n, S n = n + 1.
Proof.
  induction n.
  - reflexivity.
  - simpl. rewrite IHn. reflexivity.
...
In a proof by induction we easily reach the goal:

nat_to_bin (2*bin_to_nat b) = A b

which should hold for any b : bin. For b = Z this yields:

Z = nat_to_bin (0) = A Z 

which is false. 

In summary, the proposition:

forall b, nat_to_bin (bin_to_nat (b)) = b

is saying that bin_to_nat is injective (has a left inverse), but this is not

true since Z and AZ both map to 0. 
...
Fixpoint normalize (b : bin) : bin :=
  match b with 
  | Z => Z
  | A b' => if (contains(b')) then A (normalize(b')) else normalize(b')
  | B b' => B (normalize(b'))
  end.

(* Two lemmas characterize bin_to_nat *)  
Lemma not_contains_zero: 
  forall b, false = contains(b) -> bin_to_nat(b) = 0.
Proof. 
  induction b.
  - reflexivity.
  - simpl. rewrite <- plus_n_O. 
    intros H. rewrite (IHb H). reflexivity. 
  - simpl. intros H. discriminate.
Qed.

Lemma contains_zero_not:
  forall b, bin_to_nat(b) = 0 -> false = contains(b).
Proof. 
  induction b.
  - reflexivity.
  - simpl. rewrite <- plus_n_O. intros H.
    assert(lem1: bin_to_nat b = 0).
    { destruct (bin_to_nat b). reflexivity. discriminate. }
    auto.
  - simpl. rewrite <- plus_n_O. intros H. 
    assert(lem1: false = true).
    { destruct (bin_to_nat b). discriminate. discriminate. } 
    rewrite lem1. reflexivity.   
Qed.

(* Two lemmas characterize contains of b and its normalized version *)  
Lemma contains_normalize:
  forall b, contains(b) = true -> contains(normalize(b)) = true.
Proof. 
  induction b.
  - discriminate.
  - simpl. intros H. rewrite H. simpl. rewrite (IHb H). reflexivity.
  - reflexivity.
Qed.  

Lemma normalize_contains:
  forall b, contains(normalize(b)) = true -> contains(b) = true.
Proof. 
  induction b.
  - discriminate.
  - simpl. 
...
Theorem bin_to_nat_normalized: 
  forall b,  bin_to_nat b = bin_to_nat (normalize b).
Proof. 
  induction b. 
  - reflexivity.
  - simpl. rewrite <- plus_n_O.
    destruct (contains b) eqn:contains_hyp.
    + simpl. rewrite <- plus_n_O. rewrite IHb. reflexivity.
    + simpl. rewrite <- IHb. rewrite not_contains_zero. reflexivity.
      rewrite contains_hyp. reflexivity.
  - simpl. rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite IHb. 
    reflexivity.
Qed.

Theorem nat_to_bin_pres_incr : forall (m:nat), incr(nat_to_bin m) = nat_to_bin (S m).
Proof. 
  destruct m.
  - reflexivity.
  - reflexivity.
...
Lemma double_nat: 
  forall n, n <> 0 -> nat_to_bin(n + n) = A (nat_to_bin n).
Proof. 
  induction n.
...
Theorem left_inverse_normalized: 
  forall b,  nat_to_bin (bin_to_nat (normalize b)) = normalize b.
Proof. 
  induction b.  
  - reflexivity. 
  - simpl. 
    destruct (contains b) eqn:contains_hyp.
    + simpl. rewrite <- plus_n_O. rewrite double_nat.
      ++ rewrite IHb. reflexivity.
      ++ assert(lem1: contains(normalize(b)) = true).
         { rewrite contains_normalize. reflexivity. rewrite contains_hyp. reflexivity. }
         destruct (bin_to_nat (normalize b)) eqn:H. 
          +++ assert (lem2: contains(normalize(b)) = false).
              { symmetry. rewrite <- contains_zero_not. reflexivity. rewrite H. reflexivity. }
              rewrite lem1 in lem2. discriminate.
          +++ discriminate.
    + rewrite IHb. reflexivity.
  - simpl. rewrite <- plus_n_O. rewrite <- sn_to_s. rewrite <- nat_to_bin_pres_incr.
    destruct (bin_to_nat (normalize b)).
    + simpl. symmetry in IHb. rewrite IHb. reflexivity.
    + rewrite double_nat.
      +++ simpl. rewrite nat_to_bin_pres_incr. rewrite IHb. reflexivity.
      +++ discriminate.
Qed.


Theorem left_inverse: forall b, nat_to_bin (bin_to_nat (b)) = normalize(b).
Proof. 
  intros b. rewrite bin_to_nat_normalized. rewrite left_inverse_normalized. reflexivity.
Qed.
