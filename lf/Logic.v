...
Proof.
  ...
  - destruct n. 
    + apply H. 
    + rewrite plus_Sn_m in H. discriminate.
Qed.
(** [] *)
...
(* 
  Given a proposition P, proving ~~P reduces to assuming P -> False and P
  deduce False. This can be done applying P -> False to P. 
*)
...
(* 
  The proof follows by unfolding and applying implication until 
...
Proof.
  ...
  split.
  - intros isP. 
    apply H1 in isP as H5. 
    apply H3 in H5 as H6.
    apply H6.
  - intros isR.
    apply H4 in isR as H5.
    apply H2 in H5 as H6.
    apply H6.
Qed.
(** [] *)
...
Proof.
  ...
  split.
  - induction l. 
    + simpl. intros contra. exfalso. apply contra.
    + simpl. intros disj. destruct disj.
      ++ exists x. split. apply H. left. reflexivity.
      ++ apply IHl in H. destruct H as [x' H']. exists x'. 
         split. apply H'. right. apply H'.
  - intros ex. destruct ex as [x' [H1' H2']]. rewrite <- H1'.
...
Proof.
  intros A l l' a. 
  induction l. 
  - simpl. split. right. apply H. intros []. exfalso. apply H. apply H.
  - simpl. rewrite <- or_assoc. rewrite IHl. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H.
    ...
    + simpl. split. 
      ...
      ++ simpl in H. apply IHl. intros x H'. apply H. right. apply H'.
  - intros H x H'.
    induction l as [| h tl IHl].
    + simpl in H'. exfalso. apply H'.
    + simpl in H'. destruct H' as [H1' | H2'].  
      +++ rewrite <- H1'. simpl in H. apply H.
      +++ simpl in H. apply IHl. apply H. apply H2'.
Qed.
(** [] *)
...
Lemma tr_rev_gen : forall (X: Type) (l1 l2: list X), rev_append l1 l2 = rev (l1) ++ l2.
Proof. 
  induction l1.  
  - reflexivity.
  - intros l2. simpl. rewrite <- app_assoc. simpl. rewrite IHl1. reflexivity.
Qed.
...
Lemma tr_rev_correct : forall X, @tr_rev X = @rev X.
  ...
  induction x as [| h tl IHtl].
  - reflexivity. 
  - simpl. rewrite tr_rev_gen. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite evenb_S. destruct (evenb n).
    ++ simpl. destruct IHn as [k H]. exists k. rewrite H. reflexivity.
    ++ simpl. destruct IHn as [k H]. exists (S k). simpl. rewrite H. reflexivity.
Qed.
(** [] *)
...
Proof.
  destruct b1, b2.
  - simpl. split. split. reflexivity. reflexivity. reflexivity.
  - simpl. split. split. reflexivity. apply H. intros H. apply H.
  - simpl. split. split. apply H. reflexivity. intros H. apply H.
  - simpl. split. split. apply H. apply H. intros H. apply H.
Qed.
...
Proof.
 destruct b1, b2.
  ...
  - simpl. split. intros H. left. apply H. intros [H1 | H2]. apply H1. apply H2.
Qed. 
(** [] *)
...
Proof.
  ...
  - generalize dependent y. induction x.
    + destruct y. discriminate. discriminate. 
    + destruct y. 
      ++ discriminate. 
      ++ simpl. intros H.
         unfold not. intros H'. injection H' as H''. 
         apply IHx in H. apply H in H''. apply H''.
  - generalize dependent y. induction x. 
    + destruct y.
    ...
    + destruct y.
      ++ reflexivity.
      ++ simpl. intros H.
         apply IHx. unfold not. intros H'.
         unfold not in H. rewrite H' in H. 
         apply H. reflexivity.
Qed.
(** [] *)
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  | l, nil => false
  | nil, l => false
  | h1 :: tl1, h2 :: tl2 => (andb (eqb h1 h2) (eqb_list eqb tl1 tl2))
  end.
...
Proof.
  ...
  - generalize dependent l2. induction l1.
    ...
    + destruct l2. 
      ++ discriminate. 
      ++ simpl. intros IH. 
         apply andb_true_iff  in IH as IH'. destruct IH' as [IH' IH''].
         apply IHl1 in IH''.
         apply H in IH'.
  ...
  - generalize dependent l2. induction l1.
    ...
    + destruct l2. 
      ++ discriminate. 
      ++ simpl. intros IH. injection IH as IH1 IH2. 
         apply andb_true_iff. split.
         rewrite IH1. apply H. reflexivity.
         apply IHl1. apply IH2.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l.
    ...
    + simpl. split. 
      ++ apply andb_true_iff in H as [H1 H2]. apply H1.
      ++ apply IHl. apply andb_true_iff in H as [H1 H2]. apply H2.
  - induction l.
    + reflexivity.
...
(* The types are inferred so it may be that from one side the other is 
   deduced.  
...
(** **** Exercise: 3 stars, standard (excluded_middle_irrefutable)  
    ...
    implies [~ ~ P], by the exercise below (instantiate with P := P \/ ~P)), 
    which would be a contradiction.  But since we can't, it is safe to 
    add [P \/ ~P] as an axiom. *)
...
Proof.
  ...
  unfold not in negE.
  destruct (H (P x)) as [H1 | H2].
  apply H1.
  exfalso. apply negE. exists x. apply H2.
Qed.
(** [] *)
