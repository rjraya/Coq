...
Proof.
  unfold swap_pair. destruct p. reflexivity.
Qed.
(** [] *)
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | m :: t => if (Nat.odd m) then m :: oddmembers t else oddmembers t
  end.
...
Fixpoint count (v:nat) (s:bag) : nat :=
  match s with 
  | nil => 0
  | t :: tl => if (eqb t v) then 1 + count v tl else count v tl
  end. 
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with 
  | nil => nil
  | t :: tl => if (eqb t v) then tl else t :: remove_one v tl
  end. 
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with 
  | nil => nil
  | t :: tl => if (eqb t v) then remove_all v tl else t :: remove_all v tl
  end. 
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | nil => true
  | h1 :: s1' => 
    if (member h1 s2) then  
        (subset s1' (remove_one h1 s2))
    else false
  end.
...
Lemma eq_reflexive: forall (v : nat), v =? v = true.
Proof. 
  induction v. 
  - reflexivity.
...
Theorem bag_theorem : forall (b : bag) (v : nat), count v (add v b) = 1 + count v b.
Proof.
  induction b.
  - simpl. intros v. rewrite eq_reflexive. reflexivity.
  - simpl. intros v. rewrite eq_reflexive. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. intros l2. rewrite IHl1. rewrite app_assoc. reflexivity.
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | nil, nil => true
  | h1 :: l1', h2 :: l2' => andb (eqb h1 h2) (eqblist l1' l2')
  | _, _ => false
  end. 
...
Proof.
  induction l. 
  - reflexivity.
  - simpl. rewrite eq_reflexive. simpl. rewrite IHl. reflexivity.
Qed.
(** [] *)
...
Proof.
 simpl. intros d x v. rewrite <- eqb_id_refl. reflexivity.
Qed.
(** [] *)
...
(* There is no element of type baz since there is no way to construct one
   in the absence of base case. *)
